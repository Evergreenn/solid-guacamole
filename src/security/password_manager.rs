...
pub fn hash_password(clear_pass: &str)-> String {
    ...
    let password_hashed = argon.hash_password(clear_pass.to_string().as_bytes(), &salt).unwrap().to_string();
...
pub fn verify_password(clear_pass: String, hashed_pass: String)-> bool {
    ...
    argon2.verify_password(clear_pass.as_bytes(), &PasswordHash::parse(hashed_pass_str, Encoding::default()).unwrap()).is_ok()
}
